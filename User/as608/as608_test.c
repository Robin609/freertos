...
void Show_Message(void)
{
  ...
  AS608_INFO("    4     ------    清空指纹库\r\n"); 
	lcd_as608UIsomething();

}

void lcd_as608UIsomething() //指纹首界面
{
	LCD_String(20, 46,"Add     --1  ",16, YELLOW , BLACK); 
	LCD_String(20, 62,"Compare --2  ",16, YELLOW , BLACK); 
	LCD_String(20, 78,"Delete  --3  ",16, YELLOW , BLACK); 
	LCD_String(20, 94,"Empty   --4  ",16, YELLOW , BLACK); 
}

void lcd_as608UInothing()  //空白部分
{
	LCD_String(20, 46,"             ",16, YELLOW , BLACK); 
	LCD_String(20, 62,"             ",16, YELLOW , BLACK); 
	LCD_String(20, 78,"             ",16, YELLOW , BLACK); 
	LCD_String(20, 94,"             ",16, YELLOW , BLACK); 
}
  ...
  //AS608_INFO("接收到字符：%d\r\n",ch);
		...
		switch(ch)
		{
			...
			if(ch>0&&ch<13)
			{
				...
				Show_Message();
				lcd_as608UInothing();
				LCD_String(20, 62,"enter valid    mun",16, YELLOW , BLACK);
				delay_ms(3000);
				lcd_as608UIsomething();
  ...
  while(1)
  {
    switch (j)
    {
      case 0:                            /*执行第1步*/
        ...
        AS608_INFO("******命令：请按手指******\r\n");
        	LCD_String(20, 62,"Please press your finger",16, YELLOW , BLACK);
        ...
        if(sure == 0x00)
        {
          ...
          { 
				    AS608_INFO("输入指纹1正常！！！\r\n");
            LCD_String(20, 62," enter fingerprint1 succeed ",16, YELLOW , BLACK);
            sure=PS_HighSpeedSearch(CHAR_BUFFER1,0,PS_MAXNUM,&ID);
            if(sure==0x00)
            {
             AS608_INFO ("指纹已存在，指纹ID：%d！！！\r\n\r\n",ID);
             	LCD_String(20, 62,"fingerprint is existed",16, YELLOW , BLACK);
             return ;
            }              
            else 
            {
      ...
      case 1:
        ...
        AS608_INFO("******命令：请再按一次手指******\r\n");
      	LCD_String(20, 62,"again press ",16, YELLOW , BLACK);
        sure=PS_GetImage();
        if(sure==0x00)
        { 
          ...
          { 
				    AS608_INFO("输入指纹2正常！！！\r\n");
						LCD_String(20, 62," enter fingerprint2 succeed ",16, YELLOW , BLACK);
            i=0;
            j=2;                         /*跳转到第3步*/
          }
				  else 
       ...
       case 2:
         ...
         {
				   AS608_INFO("输入指纹对比成功！！！\r\n");
           	LCD_String(20, 62,"The input comparison is successful",16, YELLOW , BLACK);
           j=3;                         /*跳转到第4步*/
         }
         else
         {
           AS608_INFO("对比失败，请重新录入指纹！！！\r\n");
					 LCD_String(20, 62," fail,press again ",16, YELLOW , BLACK);
           ShowErrMessage(sure);
           i=0;
           j=0;
         }
       ...
       case 4:
         ...
         { 
           ...
           AS608_INFO ("******命令：请输入存储ID，范围为0—239******\r\n");
          	LCD_String(20, 62,"enter mun for fingerprint",16, YELLOW , BLACK);
           ID=GET_NUM();
         }while(!(ID<PS_MAXNUM));
         ...
         if(sure==0x00) 	
         {
           AS608_INFO("录入指纹模块成功！！！\r\n\r\n");
					 LCD_String(20, 62,"                                            ",16, YELLOW , BLACK);
   				 LCD_String(20, 62," success  ",16, YELLOW , BLACK);
      ...
      { 
        AS608_INFO("录指纹失败！\r\n\r\n\r\n");
        LCD_String(20, 62," fail enter",16, YELLOW , BLACK);
